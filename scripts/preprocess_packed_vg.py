...
parser.add_argument('--min_objects_per_image', default=16, type=int)
parser.add_argument('--max_objects_per_image', default=128, type=int)
parser.add_argument('--max_attributes_per_image', default=1000, type=int)
...
parser.add_argument('--max_relationships_per_image', default=128, type=int)
...
def main(args):
    ...
    create_rel_vocab(args, train_ids, relationships, object_id_to_obj, rel_aliases, vocab)
...
def encode_graphs(args, splits, objects, relationships, vocab,
    ...
    for split, image_ids in splits.items():
        ...
        for image_id in image_ids:
            ...
            for object_id in image_object_ids:
                ...
                if attributes is None:
                    ...
                    attributes_obj_mapping.append([-1] * args.max_attributes_per_image)
                    ...
                    attributes_obj_mapping.append(attribute_ids)
            ...
            real_object_attributes.append(attributes_obj_mapping)
        ...
        numpy_arrays[split] = {
            ...
            'attribute_ids': np.asarray(attribute_ids),
            'real_object_attributes': np.asarray(real_object_attributes),
        }
...
if __name__ == '__main__':
    ...
    args.images_json = os.path.join(args.dataroot, "vg", 'image_data.json')
    args.objects_json = os.path.join(args.dataroot, "vg", 'objects.json')
    args.attributes_json = os.path.join(args.dataroot, "vg", 'attributes.json')
    args.object_aliases = os.path.join(args.dataroot, "vg", 'object_aliases.txt')
    args.relationship_aliases = os.path.join(args.dataroot, "vg", 'relationship_alias.txt')
    args.relationships_json = os.path.join(args.dataroot, "vg", 'relationships.json')
    args.relationships_json = os.path.join(args.dataroot, "vg", 'relationships.json')
    args.relationships_json = os.path.join(args.dataroot, "vg", 'relationships.json')
    args.output_vocab_json = os.path.join(args.dataroot, "vg", 'vocab.json')
    args.output_h5_dir = os.path.join(args.dataroot, "vg")
    main(args)
