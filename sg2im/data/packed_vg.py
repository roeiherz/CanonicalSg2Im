...
from sg2im.data.utils import Resize, encode_image


class PackedVGSceneGraphDataset(BaseDataset):

    def __init__(self, h5_path, base_path, mode, image_size=(256, 256), mask_size=0, normalize_images=True,
                 min_objects=0, max_objects=1000, max_samples=None, include_relationships=True,
                 use_orphaned_objects=True, use_transitivity=False, learned_transitivity=False, include_dummies=True,
                 learned_symmetry=False, use_converse=False, learned_converse=False):
    ...
    def __getitem__(self, index):
        ...
        obj_centers = torch.stack([boxes[:, 0] + (boxes[:, 2] / 2), boxes[:, 1] + (boxes[:, 3] / 2)], dim=1)
